...
modified_chg=data['Value1'].to_frame()
for i in range(len(data_steady['index'])):
	modified_chg.at[data_steady['index'][i],'Value1']=data_steady['modified_chg'][i]
for i in range(len(data_jumpy['index'])):
	modified_chg.at[data_jumpy['index'][i],'Value1']=data_jumpy['modified_chg'][i]
#输出涨跌幅序列文件
#modified_chg.to_csv('modified_chg.csv',index=False)
data['Value1']=modified_chg['Value1']
...
df={'index': lis,\
'ATR':[ATR(i) for i in lis],\
'Date':[data['Date'][i] for i in lis],\
'modified_chg':[data['Value1'][i] for i in lis],\
'chg':[data_og['Value1'][i] for i in lis],\
'period_flunc':lis_,\
'c10':[(data['Value2'][i+10]-data['Value2'][i])/data['Value2'][i] for i in lis],\
'c20':[(data['Value2'][i+20]-data['Value2'][i])/data['Value2'][i] for i in lis],\
'c40':[(data['Value2'][i+40]-data['Value2'][i])/data['Value2'][i] for i in lis]}
