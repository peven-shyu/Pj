...
data['Value2'] = data['Value2'].str.replace(',', '').astype(float)
data_copy = data.copy()
...
ax.xaxis.set_major_locator(mdates.DayLocator(interval=160))
...
for signal_date in signal_dates[0:1]:
    ax.axvline(signal_date,0,data_copy['Value2'][signal_date]/10000, c='r', linewidth=0.6, label='信号日')
for signal_date in signal_dates[1:]:
    ax.axvline(signal_date,0,data_copy['Value2'][signal_date]/10000, c='r', linewidth=0.6)
...
ax.set_title('北向异常信号分布', fontsize=23)
